...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            ...
            # check if action node is reachable from the previous level
            if node_a.prenodes.issubset(self.s_levels[level]):
                # now append the node and connect the children and parents
    ...
    def add_literal_level(self, level):
        ...
        for node_a in self.a_levels[level - 1]:
            ...
            # go thru preceding level s nodes
            for node_s in node_a.effnodes:
                # add the s node to the s_nodes set and connect the parents and children
                s_nodes.add(node_s)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # check for positive effects in a1 also in negative effects of a2
        for pos_effect in node_a1.action.effect_add:
            if pos_effect in node_a2.action.effect_rem:
                return True

        # check for positive effects in a2 also in negative effects of a1
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # check for negative effects in a1 not in positive effects in a2 positives
        for neg_effect in node_a1.action.effect_rem:
            if neg_effect in node_a2.action.precond_pos:
                return True

        # check for negative effects in a2 not in positive effects in a1 positives
        for neg_effect in node_a2.action.effect_rem:
            if neg_effect in node_a1.action.precond_pos:
                return True

        # check for positive effects in a1 not in negative effects in a2 negatives
        for pos_effect in node_a1.action.effect_add:
            if pos_effect in node_a2.action.precond_neg:
                return True

        # check for positive effects in a2 not in negative effects in a1 negatives
        for pos_effect in node_a2.action.effect_add:
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # return true if any a1 parents and a2 parents are mutex
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        # return true if the node symbols are equal and the signs are not the same
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        # return true if any combination of nodes are mutex
